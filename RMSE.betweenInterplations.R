...
my_rmse[k-12] <- rmse(flex2$conc, flex$conc, na.rm = FALSE)
my_rmse2[k-12] <- rmse(flex3$conc, flex2$conc, na.rm = FALSE)
my_rmse3[k-12] <- rmse(flex$conc, flex3$conc, na.rm = FALSE)
}

j<-13:length(icon.flist)
df1 <- data.frame(j,rmse=my_rmse)
df2 <- data.frame(j,rmse=my_rmse2)
df3 <- data.frame(j,rmse=my_rmse3)
df1$model="nnb-rbf"
df2$model="byc-rbf"
df3$model="byc-nnb"
df =rbind(df1,df2,df3)
#browser()
png(paste0("/project/ivme/MCH-1/icon-art-BRM-CDO/scripts/rmse3Interpolations211218_capture.png"), height=12, width=24, units="cm", res=800)
# plot(j, my_rmse,xlab="Time", ylab="RMSE", col = "red")
g <- ggplot(df, aes(x=j,y=rmse))+
	geom_line(aes(color=model))+	# basic graphical object
         ylab("RMSE") + xlab("Time")
print(g)
dev.off()
  
#prior_sim <- data.frame(samples = c(my_rmse,my_rmse2,my_rmse3),
 #       priors = rep(c("nnb-rbf","byc-rbf","byc-nnb"), each = 13)) 
