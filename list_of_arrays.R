...
#plume.stats <- array(c(levels,flex.vertical.mean,icon.vertical.mean), dim=c(33,3))
...
# for (ii in 1:nn.t){
	...
	msk = which(flex$zz>0 & icon$zz>0)
        print("apply(flex$zz,1:2,mean)")
	print(apply(flex$zz,1:2,mean))
	flex.vertical.mean = apply(flex$zz,1:2,mean)
	icon.vertical.mean = apply(icon$zz,1:2,mean)
	plume.stats <- array(c(levels,flex.vertical.mean,icon.vertical.mean), dim=c(33,3))
        print("icon.vertical.mean")
	print(icon.vertical.mean[1:33])
# }
