...
icon.dir = "/project/ivme/MCH-1/icon-art-BRM/icon_output"
icon.fn = "ICON-ART-OEM_1504NewICBC_DOM01_00140000.nc"          #       file name gives time after release
icon.var = "testtr12"

flex.dir = "/project/ivme/MCH-1/icon-art-BRM/rbf_interpolation/150418/newICBC/"
...
lev = 10		#	level to be plotted, counted from the bottom
...
flex = read.grid.output(file.path(flex.dir, flex.fn), release=rel.com, tidx=tidx)
...
fill.2dplot(flex, para="conc", xlim=xlim, ylim=ylim, zlim=zlim, lev=lev, log.scale=TRUE, 
	borders=TRUE, type="grid", color.palette=special.rainbow, map.db="worldHires", plot.grid=TRUE, 
	nlevels=30, pnts=release)
...
#	single plot for both models with one color scale
 png(paste0("/project/ivme/MCH-1/icon-art-BRM/images/test_locBRM_",level,".png"), height=12, width=24, units="cm", res=800)
layout(matrix(1:3, ncol=3), widths=c(1,1,lcm(2)))
par(mar=c(4,4,2,1)+.1)
col.scale= fill.2dplot(flex, para="conc", xlim=xlim, ylim=ylim, zlim=zlim, lev=lev, log.scale=TRUE,
    borders=TRUE, type="grid", color.palette=special.rainbow, map.db="worldHires", plot.grid=TRUE, 
