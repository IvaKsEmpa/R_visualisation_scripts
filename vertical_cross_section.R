...
icon.dir = file.path("/project/ivme/MCH-1/icon-art-BRM/icon_scripts_output_code/icon_art_output", 
	case, "structured")
icon.pattern = "ICON-ART-OEM_250518CDO_DOM01_[0-9]*.nc"
...
palette.par = list(name="YlOrRd")
color.palette = brewer.ramp
altlim = c(10,2000)
dalt = 10

levels = c(5,15)		#	level to be plotted, counted from the bottom
...
rel.idx = which(flex.hdr$rel.com==rel.com)
outlev.mids = flex.hdr$outheight - diff(c(0, flex.hdr$outheight))/2 
xlim = c(-1, 1) *   1 + flex.hdr$rel.lng1[rel.idx]
ylim = c(-1, 1) * 0.5 + flex.hdr$rel.lat1[rel.idx]
flex.out.dtm = flex.hdr$dtm + 
	seq(flex.hdr$loutstep, 46800, by=flex.hdr$loutstep)/86400
...
#	release location 
release = list(x=flex.hdr$rel.lng1[rel.idx], y=flex.hdr$rel.lat1[rel.idx], 
	z=flex.hdr$rel.zagl1[rel.idx], pch=4, cex=2, col="black")
...
for (ii in 1:nn.t){
	...
	#	mean vertical cross section with longitude
	flex.xz = regularise.grid(apply(flex$zz, c(1,3), mean), 
    	xrng=flex$xrng, ylevels=c(0, flex$zrng), normalise=FALSE, ylim=altlim, dy=dalt)
	icon.xz = regularise.grid(apply(icon$zz, c(1,3), mean), 
    	xrng=icon$xrng, ylevels=c(0, flex$zrng), normalise=FALSE, ylim=altlim, dy=dalt) 

	#	plot them
	fn.png = file.path(out.dir, interpolation,
	    paste0("mean_concentration_xz_", interpolation, "_",
	    chron.2.string(flex.out.dtm[ii], "%Y%m%d_%H")) )
	open.graphics.device(fn.png, width=15, height=7.5, pointsize=10)
	par(lwd=0.5, mar=c(3,3,1,1)+.1)
	
	layout(matrix(1:3, ncol=3), widths=c(1,1,lcm(2)))
	fill.2dplot(flex.xz, log.scale=TRUE, zlim=zlim.vert, color.palette=color.palette,
	    palette.par=palette.par, plot.key=FALSE, plot.grid=TRUE, nlevels=30,
	    xlab="Longitude (°E)", ylab="Altitude agl (m)")
	textbox(par("usr")[1], par("usr")[4], "FLEXPART", adj=c(0,1), fill="white")
	textbox(par("usr")[2], par("usr")[4], chron.2.string(flex.out.dtm[ii]), adj=c(1,1))
	points(release$x, release$z, pch=release$pch, cex=release$cex, col=release$col)
	points(flex$cntr.lon, flex$cntr.height, pch=3, cex=1.5, col="gray80", lwd=2*par("lwd"))
	
	col.pal = fill.2dplot(icon.xz, log.scale=TRUE, zlim=zlim.vert, color.palette=color.palette,
	    palette.par=palette.par, plot.key=FALSE, plot.grid=TRUE, nlevels=30,
	    xlab="Longitude (°E)", ylab="Altitude agl (m)", key.title = "C (ng/kg)")
	textbox(par("usr")[1], par("usr")[4], "ICON", adj=c(0,1), fill="white")
	points(release$x, release$z, pch=release$pch, cex=release$cex, col=release$col)
	points(icon$cntr.lon, icon$cntr.height, pch=3, cex=1.5, col="gray80", lwd=2*par("lwd"))
	plot.colorpalette(col.pal)
